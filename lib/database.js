...
class Database {
	 ...
	 */
	model(name, indexes=null) {
		let Model = model(this, name);

		if(indexes) {
			if(this.connected()) {
				Model.createIndexes(indexes).then(() => {}).catch(e => console.error('[DB] Could not create indexes', e));
			} else {
				this.__indexes = this.__indexes || [];
				this.__indexes.push({
					Model,
					indexes
				});
	...
	/**
	 * Starts a transaction in a new session
	 * @param {Object} options - Options for the new transaction
	 * @returns {Transaction}
	 */
	transaction(options = null) {
		if(!this.__connected) {
			throw new Error('[DB] Cannot start transaction in disconnected database');
		}

		return new Transaction(this.__mongo, options);
	}
	...
	connect(options = null) {
		...
		let {protocol='mongodb', endpoint, username, password, port=27131, database, timeout, query='', ...other_options} = options;
		...
		let connection_url = `${protocol}://${username}:${password}@${endpoint}${protocol
			? (protocol.indexOf('srv')>-1 ? '' : `:${port}`)
			: `:${port}`}/${database}${query}`;
		...
		return Mongo.connect(connection_url, connection_options).then(client => {
			...
			if(this.__indexes) {
				this.__createIndexes().then(() => {
					delete this.__indexes;
				});
			}
	...
	/**
	 * Creates an event listener for a given event. Forwarded to mongo client.
	 * @param {string} event - Name of the event
	 * @param {func} callback - Callback function to call when event is triggered
	 */
	on(event, callback) {
		if(!this.connected()) {
	...
	clear() {
		if(!this.__client) {
			throw new Error('[DB] Cannot clear database without connecting to it first');
		}
	...
	disconnect() {
		if(!this.connected() || !this.__mongo) {
			let e = new Error('[DB] Cannot close connection with a disconnected database, use db.connect() first');
	...
	__collection(model) {
		if(!this.__client) {
			throw new Error('[DB] Cannot query collection from a disconnected database, use db.connect() first');
		}
	...
	__createIndexes() {
		let promises = this.__indexes.map(({ Model, indexes }) => {
			return Model.createIndexes(indexes, { bulk: true });
		});

		return Promise.all(promises).then(() => {}).catch(e => {
			console.error('[DB][Indexes] Could not create all indexes', e);
		});
	}
}
