...
const { StaticEventEmitter } = require('../utils/event-emitter');
...
class Model extends StaticEventEmitter {
	...
	update(data={}, { validate=true, event='update', ...options } = {}) {
		if(!this._id) {
		...
		return this.constructor.collection.updateOne({
			...
			// Emit event for save/update/delete
			// TODO: documentate every event
	...
	save(options={}) {
		if(!this._id) {
			...
			return this.constructor.collection.insertOne(this.toDataBase(), options).then(result => {
	...
	hard_delete(options={}) {
		return this.constructor.collection.deleteOne({
			...
			this.constructor.emit('hard_delete', this, {
				metadata: {
					result
				}
			});
