	...
	validate(value) {
		if(value instanceof Date) {
			if(Number.isNaN(value.getTime())) {
				throw new Error(`Invalid date object: ${value}. Consider passing strings to make debugging easier.`);
			}
			return true;
		}
...
// Object field requires all keys to be valid
...
// Like an object field, but does not validate root keys
class MapField extends GenericField {
	...
	validate(value, values, name) {
		...
		if(value instanceof Map) {
			validated_value = Object.fromEntries(value);
		}
		// Auto valid if no shape necessary
		if(Object.prototype.toString.call(validated_value) !== '[object Object]') {
			throw new Error('MapField only accepts objects (dicts) of type { [key1]: valueType, [key2]: valueType, ...etc }');
		}
		...
		for(const [k, v] of Object.entries(validated_value)) {
			// Should throw automatically, but we'll check false just in case
			const valid = this.of.is_valid(v, name, values);
			if(valid === false) {
				throw new Error(`Invalid key/value pair ${k}: ${v}`);
			}
		}
